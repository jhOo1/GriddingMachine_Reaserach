...
数据质量控制位于标准化处理与公开发布之间，其目标不仅是判断文件能否打开，还要确认数组结构、空间语义、数值语义和溯源信息均符合表2。本文将检查分为四层。第一层为配置检查，在读取大文件前验证 YAML schema、字段类型、数组长度、互斥关系和标签格式。第二层为文件结构检查，确认 NetCDF 包含 `lon`、`lat`、可选 `ind` 维度及相应坐标变量，并检查 `data` 与可选 `std` 的维度和形状。第三层为坐标与数值检查，验证经纬度范围、方向、间距、数据有效范围、缺失值掩膜和 gap-filling 结果。第四层为溯源检查，确认来源、引用、许可、单位、处理历史、生产代码版本和配置哈希完整。

当前基线中的 `verify_processed_data!` 已实现部分结构和数值检查，包括必要维度与变量、`data` 各维长度、有效范围以及特定分辨率下的陆地缺失值；但尚未系统检查坐标值、`std` 一致性、单位、引用、许可和配置版本。论文实验版本将把上述检查改为无交互、可重复的自动测试。空间方向图仍用于审核人员快速识别全球图像倒置或平移错误，但审核结果需要记录时间和审核人，且不能替代数组级金标准断言。

#### 2.3.2 标签、镜像与完整性登记

质量控制通过后，数据产品按唯一标签命名并进入待发布区，而不是直接写入正式目录。发布程序首先计算文件字节数和 SHA-256，随后将同一文件上传至至少一个公开镜像；用于论文可靠性实验的受控数据子集同时配置机构镜像和公共镜像。每个远端对象上传完成后，需要通过实际协议重新检查对象存在性、字节数和内容哈希。只有内容一致的 URL 才能登记在同一标签下，服务器能够响应 ICMP ping 既不是发布成功的充分条件，也不是判断 HTTP/FTP 文件可用的必要条件。

`Artifacts.yaml` 的单个条目至少包含标签、相对路径、镜像 URL、`SIZE_BYTES` 和 `SHA256`，目录本身还应包含 schema 版本、目录版本和生成时间。标签冲突、镜像内容不一致、缺少公共 URL 或任何自动质控失败时，发布事务立即终止，原有正式目录保持不变。当前 `wyujie` 基线的目录含1 179个标签和1 251个 URL，其中72个标签具有多个 URL，但尚无哈希字段；因此本文后续关于多镜像可靠性的实验只评价完成完整性迁移并具有受控双镜像的数据子集，不把全库表述为已经实现镜像冗余。

#### 2.3.3 事务式发布与动态目录更新

正式发布被设计为一个有明确提交点的事务：本地质量控制、生成完整性信息、上传镜像、远端复核、构建候选目录、目录 schema 校验、客户端冒烟测试和目录发布依次执行。候选 `Artifacts.yaml` 在临时位置生成，验证通过后才替换正式版本，同时保存上一版本和发布记录以支持回滚。发布记录关联数据标签、源 YAML、配置哈希、生产端 commit、文件哈希、镜像校验结果、审核人和测试平台，从而使论文中的每个数据案例能够追溯到不可变输入与代码版本。

客户端通过稳定的 Zenodo API、概念 DOI 或显式版本端点发现目录，而不依赖网页 HTML 文本结构。新目录先下载到临时文件，完成 YAML 解析、schema、版本和完整性检查后原子替换本地目录；任何阶段失败时继续使用上一可用版本。目录更新与 `GriddingMachine.jl` 软件发布解耦，因此新增或修订数据不要求用户升级客户端，但客户端必须拒绝高于其支持范围的 schema 版本。

当前基线尚未形成上述闭环：上传和目录脚本仍含服务器路径及记录号硬编码，FTP 上传尚未实现，部分网络请求关闭 TLS 校验，URL 检查仅遍历目录前10个标签，客户端目录更新通过解析 Zenodo HTML 并直接覆盖本地文件。这些代码只作为架构原型，不作为论文结果。只有发布门禁、故障回滚和三平台冒烟测试全部通过的冻结版本，才用于第3章的验证实验。
