...
2022版GriddingMachine的主要贡献是建立统一的网格和变量约定，并通过标签及Julia artifact降低多源全球数据的发现和调用成本[4]。当前更新进一步连接新数据贡献、目录与镜像维护、统一读取和模型初始化数据组织。其关键变化不是TOML换成YAML，也不是单纯增加数据数量，而是共享配置契约、独立可验证目录、失败不污染正式文件的下载状态机，以及可回归测试的模型输入接口。当前论文分支已补齐schema、显式维度重排、目录事务替换、`SIZE/SHA256`校验和主要函数回归，并以14个真实陆面文件完成`gm2`链路预实验；尚未完成的是完整生产矩阵、历史目录迁移、三平台真实网络、陆面逐字段金标准和真实气象链路。

阶段性结果表明核心机制已具有自动回归证据，但尚不足以回答全部RQ1—RQ4。生产正确性仍需逐点金标准和网页—流水线端到端契约；贡献流程需由未参与开发者复现；分发可靠性需补三平台真实协议故障；模型接口虽已有真实陆面链路证据，仍需补陆面逐字段金标准和真实气象链路。这样的证据分层避免用功能列表、单次成功运行或主观代码量替代验证。
...
当前 Windows/Julia 1.12.6 环境中，GriddingMachineDatasets既有67项、GriddingMachine 61项以及Emerald合成输入最小烟雾5项测试全部通过，证明上述核心机制在受控夹具下可运行。两个内部压缩样本的Windows暖缓存预实验支持直接NetCDF减少额外处理时间和逻辑临时占用；Windows包级真实HTTP故障矩阵65/65满足预期状态，支持镜像回退、完整性拒绝和失败不污染正式文件。14个真实`gm2`陆面文件全部通过完整性核验，US-NR1参数字典成功生成，撒哈拉非植被格点按预期拒绝。正式结论仍需补齐大文件、缓存条件、锁定依赖环境和跨平台复核。投稿前还须完成31案例生产流水线、Linux/macOS故障注入、真实镜像、陆面逐字段金标准、真实气象链路及贡献者复现实验；在这些证据缺失时，本文不声称初测效率数值可跨平台外推、镜像排序跨平台最优、真实参数已完成科学数值验证或接口已验证长期模型模拟。
