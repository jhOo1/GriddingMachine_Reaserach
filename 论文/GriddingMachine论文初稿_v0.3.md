...
地球系统模式依赖区域至全球尺度的网格数据进行参数初始化和结果验证。尽管当前数据大多采用NetCDF、HDF或GeoTIFF等标准格式，源数据处理规则、单位换算、数据标识和网络分发方式仍存在差异，研究人员往往需要为不同模式重复维护数据整理和读取程序。本文在2022版GriddingMachine基础上，完善数据预处理、数据分发和数据读取流程：以共享YAML配置描述源数据结构及转换规则，以独立目录管理标准化数据的标签、镜像和完整性信息，并通过统一接口组织陆面参数和气象驱动。除Julia外，本文适配MATLAB、Octave、Python和R对新版数据的自动获取，并新增C和Fortran的自动获取入口。OISST V2.1标准化后与独立参考逐点一致；ELEV和LAI的直接NetCDF读取降低了外层归档带来的访问开销；代表性数据在FTP和Zenodo镜像之间保持文件大小和SHA-256一致；14类陆面数据和8类ERA5逐时数据能够形成US-NR1的模式输入并完成Emerald初始化和60 s首步。结果表明，更新后的GriddingMachine能够减少多源网格数据在预处理、获取、检查和模式接入过程中的重复工作，为地球系统模式研究提供可复用的数据组织和调用基础。
