...
Wang等[4]于2022年发布了基于Julia语言的GriddingMachine数据库和软件，旨在将常用于陆面和地球系统模拟的全球数据处理为具有统一空间和变量约定的NetCDF文件，并通过标签、Artifacts.toml文件和Julia artifact机制实现数据管理与自动下载，同时提供Julia、MATLAB、Octave、Python和R接口。2022版已经建立了统一网格约定、数据标签和多语言访问基础，但其数据处理主要依赖数据集专用脚本，标准化数据以带有标签文件的`tar.gz`归档分发，数据目录也随软件包维护。随着数据类型、分发位置和模式应用链条扩展，源数据处理规则、数据目录、网络获取和模式输入之间需要进一步统一。
