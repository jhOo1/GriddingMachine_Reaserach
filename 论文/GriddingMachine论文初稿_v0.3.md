...
本文在 2022 版 GriddingMachine 基础上，更新数据预处理、数据分发和数据读取流程，形成数据生产与标准化、数据目录与获取、模式输入组织三个相互衔接的环节。数据生产与标准化层以共享 YAML 配置规范表达源维度、坐标、数值变换和缺失值填补（gap filling）及质量控制；数据目录与获取层以逻辑标签连接数据副本，并对登记了文件字节数和 SHA-256 的条目执行内容核验；模式输入组织层通过统一读取、陆面参数融合和气象驱动组织形成满足模式接口要求的数据结构。
