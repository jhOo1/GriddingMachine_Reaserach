...
该设计将各语言的调用方式与下载逻辑分开维护，使科研人员可以在已有程序中自动获取文件，再使用相应语言的NetCDF工具读取数据。第3—4节的事务式目录管理、镜像获取和模式输入实验采用Julia主流程；非Julia接口的源码和调用说明随代码仓库的clients目录提供。
