...
`GriddingMachine.jl`的Collector显式配置数据根目录、目录来源和本地目录文件。远端目录先下载至临时路径，经根节点和字段schema校验后再替换正式目录，并保留上一有效版本。目录条目以标签为键，核心字段包括安全相对路径`PATH`、一个或多个`URL`以及可选的`SIZE`与`SHA256`。严格完整性模式要求后两项同时存在；兼容模式允许历史条目继续访问，因此本文不把当前历史目录描述为已经全部完成完整性字段迁移。
...
这一设计把“从哪里获取”与“获取到的内容是否是登记产品”分成两个判断：镜像与延迟信息负责尝试顺序和回退，目录中的产品身份与完整性字段负责严格核验。本文的多镜像实验只针对配置了相应镜像和完整性元数据的代表性条目，不据此宣称历史目录已经实现全库冗余。相关公共接口、状态恢复命令和完整目录示例列入补充材料。
