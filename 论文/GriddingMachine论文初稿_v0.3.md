...
在数据使用端，目录管理模块显式设置数据根目录与目录来源。目录更新经临时文件完成配置规范校验和事务式替换，并保留上一有效版本。产品获取模块提取各镜像地址的主机名，以可获得的平均往返延迟辅助确定候选顺序，同时保留全部镜像并依次回退。每次传输创建独立临时文件。对于目录中已登记`SIZE`和`SHA256`的产品，文件只有在字节数和摘要核验通过后才进入正式数据目录；严格完整性模式要求两项字段同时存在，兼容模式用于承接尚未回填完整性元数据的历史条目。本文将版本化目录、内容一致的镜像、事务式落盘和可选的严格内容核验共同构成的获取机制定义为“可信分发”。这里的“可信”限定为具备完整性元数据的下载对象能够与目录登记的标准产品进行内容身份核对，不延伸表示数据源真实性、产品科学正确性或网络服务的长期可用性。

数据读取层提供标签驱动的整场、指定周期及站点读取，并进一步完成格点尺度陆面参数融合与气象驱动组织。由14类产品组成的第二套陆面参数集合（代码标识为`gm2`）贯通参数组织链路并进入Emerald初始化。本文所称“模型就绪”特指标准产品经过格点索引、字段组织、时间轴构建和量纲衔接后能够满足Emerald输入接口并完成初始化及首步运行，不用于表示对Emerald全部科学过程或长期积分性能的验证。
...
二维数据贡献示例采用`(lat,lon)`源排列、北向南纬度和`0～360°`经度，配置声明源维度、纬度翻转、经度半球转换和线性变换`2x+1`，最终生成标准`(lon,lat)`产品，逐点结果与独立参考数组一致。该实例用于说明“配置意图—执行转换—标准产品”之间的对应关系；完整YAML、字段字典、命令和目录组织移列补充材料S3及版本化贡献指南。
...
`GriddingMachine.jl`的Collector显式配置数据根目录、目录来源和本地目录文件。远端目录先下载至临时路径，经根节点和字段schema校验后再替换正式目录，并保留上一有效版本。目录条目以标签为键，核心字段包括安全相对路径`PATH`、一个或多个`URL`以及可选的`SIZE`与`SHA256`。严格完整性模式要求后两项同时存在；兼容模式允许历史条目继续访问。当前目录中部分历史条目尚未回填完整性字段或新增公共镜像，主要反映文件上传、镜像复制和目录登记的发布进度，而不是目录schema、校验逻辑或多镜像机制的能力上限。
...
这一设计把“从哪里获取”与“获取到的内容是否是登记产品”分成两个判断：镜像与延迟信息负责尝试顺序和回退，目录中的产品身份与完整性字段负责严格核验。本文的多镜像实验只针对已经完成镜像上传、目录登记和完整性元数据回填的代表性条目；尚未登记第二URL的历史产品可在完成镜像上传后直接加入同一目录机制，无需修改获取框架。相关公共接口、状态恢复命令和完整目录示例列入补充材料。
...
为避免把受控测试、真实数据和真实网络观测混为同一证据层级，表3先给出全文核心主张、证据类型和结论边界。跨操作系统持续集成作为横向验证维度，贯穿生产、分发和模型接口的确定性核心路径。

**表3 GriddingMachine框架核心主张、证据层级与结论边界**

| 核心主张 | 受控或确定性证据 | 真实数据/网络证据 | 主要结论边界 |
|---|---|---|---|
| 异构源数据可按共享生产契约转换为标准产品 | 31组受控实例、ELEV重复生产与逐点核对 | OISST V2.1真实异构源产品 | 支持所测试全球规则经纬网结构及其显式源维度、坐标和数值变换，不外推至所有非规则网格或复杂投影 |
...
| 标准产品可组织为本文定义的模型就绪输入 | 确定性读取、字段、时间轴与接口核对 | US-NR1的14类陆面产品、8类ERA5及Emerald初始化和60 s首步 | 验证输入组织和接口运行，不代表Emerald全部科学过程、模拟精度或长期积分稳定性 |
| 固定依赖下核心实现具有跨操作系统运行一致性 | Windows、macOS、Linux持续集成 | Windows/macOS独立性能观测及按实际环境记录的真实网络案例 | 支持所测试代码路径的平台一致性，不等同于广义科学可复现性或不同网络环境下的相同性能 |
...
OISST案例说明了这种重构在生产端的具体作用。源变量的单例维度、存储缩放和经度范围并未被隐藏在一次性脚本中，而是由源适配与共享配置共同表达；输出再通过独立解码参考核对坐标、有效值掩膜和逐点物理值。31组受控实例用于覆盖维度、坐标、数值和Gapfill等基本操作，真实OISST则检验这些机制能否共同作用于实际异构产品。两类证据的角色不同：前者用于定位处理规则是否正确，后者用于验证一条真实源数据到标准产品的组合路径。本文因此把新版的生产改进理解为“处理规则可执行和可核对”，而不是宣称消除所有地学数据异构性。

### 5.2 直接NetCDF分发与完整性保证的重新建立

直接NetCDF取消了标准文件外层`tar.gz`的打包和解包步骤。对于本文所测、已经采用NetCDF内部压缩的ELEV和LAI，外层归档仅减少2.16%～6.43%的传输字节，而直接NetCDF在两个独立环境中均缩短了缓存预热后的端到端时间，并将逻辑临时占用降低约一半。这一结果说明，对于内部已经压缩、下载后需要立即读取的单体NetCDF，分发单元的选择需要同时考虑传输字节、解包开销和首次读取路径；本文的性能结果不用于推断所有文件格式、压缩方式或广域网络条件下直接NetCDF都具有相同优势。
...
ERA5降水V1→V1_R1修订展示了独立目录的另一项作用：模型侧继续使用原逻辑标签，而物理制品、URL及完整性摘要可在目录层更新。这样的“稳定逻辑身份—可修订物理对象”并不消除版本管理责任，但减少了数据修订对下游接口的直接扰动，并使修订后的具体bitstream能够通过新的摘要重新锁定。

### 5.3 科学质量控制与内容完整性的职责边界

本文的生产质量控制和分发完整性核验回答的是两个不同问题。生产阶段通过维度、坐标方向、变量、数值范围、Gapfill、空间方向图和独立参考等检查，判断标准产品是否按照声明的处理规则生成；分发阶段的文件字节数和SHA-256只能判断下载对象是否与目录登记的标准文件保持相同内容。后者不能证明数据源本身真实、处理规则在科学上最优，也不能替代变量物理意义和量纲的审计。OISST的逐点独立参考和ERA5降水单位修订分别说明，科学内容正确性仍需要在生产和应用层依据变量语义单独检查。

基于这一边界，本文所称“可信分发”应理解为内容身份和完整性可核验的分发机制。多镜像提高的是获取路径的冗余性，候选顺序用于优化尝试过程，对于启用严格完整性核验的条目，真正决定下载文件能否进入正式目录的是内容核验。校园网实验中FTP和Zenodo提供了内容一致的副本，但这并不意味着所有镜像在长期可达性、速度或服务稳定性上等价。该限定使分发机制的结论与实际观测范围保持一致。
...
这些结果支持的是模型输入链路和接口运行，而不是Emerald科学过程的全面验证。本文因此将“模型就绪”限定为：标准产品经过格点索引、字段组织、时间轴构建和量纲衔接后，形成满足Emerald初始化接口的数据结构，并能够完成本文测试的首步运行。当前真实案例集中于2020年US-NR1单一格点；多区域、多年份、不同生态区以及长期积分中的科学表现仍需要在具体模型研究中进一步评价。

### 5.5 跨操作系统运行一致性与相关基础设施定位

Windows、macOS和Linux持续集成中的一致结果用于检查固定依赖条件下核心数据逻辑是否受操作系统差异影响。持续集成覆盖确定性的生产、分发和接口路径，本地独立环境用于测量性能和镜像故障状态，真实网络及US-NR1案例则按照实际环境单独记录。这样的分层设计可以区分代码路径的一致性与网络性能的环境依赖，因此本文使用“跨操作系统运行一致性”描述这些结果，而不把它等同于更广义的科学可复现性。
...
本文验证仍具有明确范围。第一，当前标准产品和统一读取主要面向全球规则经纬网；非规则网格、区域投影和复杂坐标可通过专用预处理进入流程，但本文未以同等规模的真实案例评价。第二，OISST提供了一条真实异构源数据链路，不能据此推断所有海洋、陆面或大气产品都可仅依赖通用配置完成适配。第三，直接NetCDF性能实验仅覆盖ELEV和LAI两个内部压缩产品以及两个独立本地环境，真实网络实验也只覆盖本文登记的代表性镜像，因此性能结果应限制在所测条件。第四，当前历史目录仍存在未回填`SIZE`/SHA-256或仅配置单一地址的兼容条目；这主要反映部分文件尚未完成公共镜像上传、复制和目录回填的发布状态，并非框架只能支持少量完整性条目或有限镜像。本文的实证结论仍限定于已经完成相应元数据和镜像配置的产品，而目录生成和获取机制本身可在后续发布时继续批量回填完整性字段并增加镜像地址。第五，模型应用证据集中于US-NR1的2020年陆面参数与ERA5驱动，并以Emerald初始化和60 s首步作为接口层验证，尚未评价长期模拟稳定性或科学精度。最后，持久化DOI归档、更多历史产品迁移和更广泛模型适配仍属于持续维护工作。上述限制不影响本文对所测试生命周期机制的验证，但界定了结论可外推的范围。
