...
本地PPT单位修订完成后，以新增物理文件`PPT_ERA5_1X_1H_2020_V1_R1.nc`发布至机构FTP；原V1物理文件保留。正式目录和本地目录继续使用V1逻辑标签，但URL均指向R1，现有`grid_weather`接口无需改变。FTP回下载、SIZE/SHA256、NetCDF结构、8784个格点值、全新Collector首次下载与第二次缓存复用以及03_09模型首步均已通过。独立目录支持继续增加Zenodo等内容一致的公共镜像。详见`../03_09_ERA5真实气象链路实验结果.md`。
