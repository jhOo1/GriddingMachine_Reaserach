...
`run_elev_real_roundtrip.jl`使用实验3.4中已经过完整性核验的`ELEV_4X_1Y_V1.nc`，比较NetCDF底层读取、GriddingMachine整场读取和三次无损生产流水线输出；同时检查历史生产YAML是否与当前文件范围、单位和标签一致。它不声称从原始MERIT DEM源数据重建产品。
