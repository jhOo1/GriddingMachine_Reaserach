...
论文实证最初限定为 Windows。本记录在 macOS 上复现两个论文分支的核心自动回归及全部受控实验（生产矩阵、效率、故障注入），并补一轮校外真实网络观察，使论文的跨平台表述均有归档数据支撑。本文后续已增加Ubuntu核心CI证据，并于2026-08-15完成Windows校园网正式比较；本记录本身仍只描述当时的macOS执行环境和校外网络观察。
